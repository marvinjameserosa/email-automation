...
export default function TemplateEditor() {
  ...
  const [variableMenuTarget, setVariableMenuTarget] = useState<
    "subject" | "message"
  >("message");
  ...
  useEffect(() => {
    async function loadAllTemplates() {
      ...
      if (htmlTemplates.length) {
        ...
        setMessage(
          htmlTemplates[0].message || "Pick a template / Make a message"
        );
  ...
  // Autosave function
  const autoSaveCurrentTemplate = useCallback(() => {
    ...
    fetch(`/api/templates?filename=${currentTemplate.filename}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        const newTemplates: Template[] = data.templates.map((t: Template) => ({
          id: t.id,
          name: t.name,
          subject: t.subject,
          message: t.message,
  ...
  }, [selectedId, templates, subject, message]);
  ...
  function saveAsNew(name?: string) {
    const templateName =
      name ??
      prompt("Enter template name:", `Template ${templates.length + 1}`);
    ...
    fetch("/api/templates", {
      ...
      .then((data) => {
        ...
        const newTemplate = newTemplates.find((t) =>
  ...
  function applyFormatting(command: string, value?: string) {
    if (isHtmlTemplate && isEditingHtml && mainIframeRef.current) {
      ...
      const iframeDoc =
        ...
        mainIframeRef.current.contentWindow?.document;
      if (iframeDoc) {
  ...
  function insertVariable(variable: string, target: "subject" | "message") {
    ...
    if (target === "subject" && subjectInputRef.current) {
      ...
      const newValue =
        message.slice(0, start) + variableText + message.slice(end);
      setMessage(newValue);
      // Set cursor position after inserted variable
      setTimeout(() => {
        textarea.focus();
            ...
            <div className="flex flex-col gap-2 max-h-[40vh] overflow-auto">
              {templates.length === 0 && (
                <div className="text-sm text-gray-500">
                  No saved templates yet. Create one and click &quot;Save as
                  new&quot;.
                </div>
              )}
              {templates.map((t) => (
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Create and preview the email template that will be sent to
                  recipients.
                  {selectedId && (
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    title={
                      headers.length === 0
                        ? "Upload CSV first to use variables"
                        : "Insert variable"
                    }
                  >
                    <Variable size={14} />
              ...
              {headers.length > 0 ? (
                <>
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-1">
                    {headers.map((header, index) => (
                      <button
                        key={index}
                        onClick={() =>
                          insertVariable(header, variableMenuTarget)
                        }
                  ...
                  <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
                    💡 Variables will be replaced with actual values from your
                    CSV when sending emails.
                  </div>
                </>
              ) : (
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-gray-500">
                    Upload a CSV file in the Table Editor first.
                  </p>
                </div>
              )}
            </div>
